...
export function AIChatbot() {
  ...
  const { players, walletAddress, placeBid } = useScoutStore();

  const handleQuickBid = async (playerId: string, highestBid: number) => {
    if (!walletAddress) return alert("Scout identified but wallet unlinked. Connect Freighter.");
    const numAmount = highestBid + 100;
    try {
      const mockAccount = new StellarSdk.Account(walletAddress, '1');
      const tx = new StellarSdk.TransactionBuilder(mockAccount, { fee: StellarSdk.BASE_FEE.toString() })
        .addOperation(StellarSdk.Operation.payment({
          destination: 'GASOCNHNNLYFNMDJYQ3XFMI7BYHIOCFW3GJEOWRPEGK2TDPGTGOPY',
          asset: StellarSdk.Asset.native(),
          amount: "1.0"
        })).setNetworkPassphrase(StellarSdk.Networks.TESTNET).setTimeout(30).build();
        
      const signedTx = await signTransaction(tx.toXDR());
      if (signedTx.error) throw new Error(signedTx.error);
      placeBid(playerId, numAmount);
      
      setMessages(prev => [...prev, { role: 'ai', content: `SECURED. Bid of ${numAmount} XLM transmitted to Soroban ledger.` }]);
    } catch (err) {
      console.error(err);
    }
  };
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 font-mono text-xs scrollbar-thin scrollbar-thumb-electric/20 scrollbar-track-transparent">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
            <div className={`relative max-w-[90%] p-3 ${
              msg.role === 'user' 
